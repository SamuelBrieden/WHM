    ...
    public THalofit, HM_verbose, Mpc_units
    ...
    TYPE HM_tables
        ...
        !REAL(dl), ALLOCATABLE :: c1(:), c2(:), rv(:), zc1(:), zc2(:)
        REAL(dl) :: sigv, sigv100, knl, rnl, sigvRnl, sigQ3Rnl, neff, sig8z, z, dc, sig8z_cold
        INTEGER :: n
    END TYPE HM_tables
...
101                 continue
    ...
    REAL :: t1, t2
    !REAL :: Wcyl, Hgr !SB Added Wcyl, Hgr
    !COMPLEX(dl) :: z_test, chg !SB added this line
    TYPE(HM_cosmology) :: cosi
    TYPE(HM_tables) :: lut
    REAL(dl), PARAMETER :: pi=pi_HM
    LOGICAL, PARAMETER :: timing_test = .FALSE.
    !character(len=104) :: filename = "/home/sbrieden/CAMB/fortran/output/hm/ptable_ori_cross_spherehalo_wcyl.dat"
    character(len=104) :: filename = "/home/sbrieden/CAMB/fortran/output/hm/delta_crit_of_z.dat"
    integer :: unit_number
    integer :: io_status
    ...
    DO j=1,nz
        ...
        ELSE
            ...
            !! Open the file
            !!Either (new file create)
            !open(newunit=unit_number, file=filename, status='replace', action='write', iostat=io_status)
            !!Or (file append)
            !open(newunit=unit_number, file=filename, status='old', position='append', iostat=io_status)

            !! Check if the file opened successfully
            !if (io_status /= 0) then
            !    write(*, *) "Error opening the file."
            !    !! Handle the error as desired (e.g., exit the loop)
            !    exit
            !endif
            ...
            DO i=1,nk
                ...
                !z_test = cmplx(0.0_dl, k*100.0_dl)
                !CALL cchg(1.5_dl, 3.0_dl, 4.0_dl*z_test, chg)
                !Hgr = REAL(exp(-2*z_test)*chg)
                !if (k*100.0_dl<0.01_dl) then
                !    Wcyl = 1.0
                !else
                !    Wcyl = (2.0_dl - 2.0_dl*Hgr)/(k*100.0_dl)**2
                !end if
                !! Write k and plin to the file
                !write(unit_number, *) k, plin, p2h, p1h, pfull, Wcyl
                CAMB_Pk%nonlin_ratio(i,j)=sqrt(pfull/plin)
            END DO
            !$OMP END PARALLEL DO
            
            !! Write z and deltac to the file
            !write(unit_number, *) z, dc_Mead(z, cosi)
            !!flush(unit_number)
            !WRITE(*, *) z, dc_Mead(z, cosi)
    ...
    ELSE IF(this%imead==0 .OR. this%imead==3 .OR. this%imead==4 .OR. this%imead==5 .OR. this%imead==6 .OR. this%imead>=10) THEN
    ...
    IF(this%imead==0 .OR. this%imead==4 .OR. this%imead==5) THEN
        eta=0.
        !eta=-0.05*lut%sig8z_cold
        !eta=0.1281*lut%sig8z_cold**(-0.3644)
    ELSE IF(this%imead>=10) THEN
        eta=0.
        !eta=-0.25*lut%sig8z_cold**(0.3644)
        !eta=cosm%eta_baryon!*lut%sig8z_cold**(2.0)
    ...
    ELSE IF(this%imead==3 .OR. this%imead==6) THEN
        eta=0.1281*lut%sig8z_cold**(-0.3644)
    END IF
    ...
    IF(this%imead==0  .OR. this%imead==10 .OR. this%imead==11 .OR. this%imead==12 .OR. this%imead==13) THEN
    ...
    ELSE IF(this%imead==3 .OR. this%imead==4 .OR. this%imead==5 .OR. this%imead==6) THEN
    ...
    IF(this%imead==0 .OR. this%imead==4 .OR. this%imead==5 .OR. this%imead==10 .OR. this%imead==11 .OR. this%imead==12 .OR. this%imead==13) THEN
        !Set to 0 for the standard linear theory two halo term
        fdamp=0.
    ELSE IF(this%imead==1 .OR. this%imead==6) THEN
    ...
    IF(this%imead==0 .OR. this%imead==4 .OR. this%imead==5 .OR. this%imead==6  .OR. this%imead>=10) THEN
    ...
    FUNCTION r_nl_new(lut,deltac,sigma_at_sigmav)
    !Calculates R_nl, defined by sigma(R_nl)=1.
    TYPE(HM_tables), INTENT(IN) :: lut
    REAL(dl), INTENT(IN) :: deltac, sigma_at_sigmav
    REAL(dl) :: r_nl_new
    INTEGER, PARAMETER :: iorder=3
    INTEGER, PARAMETER :: ifind=3
    INTEGER, PARAMETER :: imeth=2

    IF(lut%nu(1)>deltac) THEN
        !This catches some very strange values that appear for odd cosmological models
        !This is a terrible fudge, but I cannot think of a better solution
        r_nl_new=lut%rr(1)
    ELSE
        !r_nl_new=exp(find(log(deltac),log(lut%nu),log(lut%rr),lut%n,iorder,ifind,imeth))
        r_nl_new=exp(find(log(1.0_dl/(1.0_dl-0.56_dl*(sigma_at_sigmav/deltac)**1.1)),log(lut%nu),log(lut%rr),lut%n,iorder,ifind,imeth))
        
    END IF

    END FUNCTION r_nl_new
    ...
    REAL(dl), INTENT(IN) :: z
    INTEGER :: i,nm
    REAL(dl) :: Dv, dc, m, nu, r, sig, mmin, mmax, sigma_sigmav
    TYPE(HM_cosmology) :: cosm
    TYPE(HM_tables) :: lut
    ...
    IF(HM_verbose) WRITE(*,*) 'HALOMOD: sigf min:', lut%sigf(lut%n)
    IF(HM_verbose) WRITE(*,*) 'HALOMOD: sigf max:', lut%sigf(1)
    ...
    lut%sigvRnl=sigmaV(lut%rnl,z,0,cosm)
    IF(HM_verbose) WRITE(*,*) 'HALOMOD: sigvRnl [Mpc/h]:', lut%sigvRnl
    lut%sigQ3Rnl=sigmaQ3(0.d0,z,0.01_dl,lut%knl,0,cosm)
    IF(HM_verbose) WRITE(*,*) 'HALOMOD: sigQ3Rnl [Mpc/h]:', lut%sigQ3Rnl
    ...
    CALL this%conc_bull(z,4,lut,cosm)
    !CALL this%conc_ludlow(z,lut,cosm)
    ...
    IF(HM_verbose) WRITE(*,*) 'HALOMOD: zc min [Msun/h]:', lut%zc(lut%n)
    IF(HM_verbose) WRITE(*,*) 'HALOMOD: zc max [Msun/h]:', lut%zc(1)
    IF(HM_verbose) WRITE(*,*) 'HALOMOD: Done'
    IF(HM_verbose) WRITE(*,*)
    IF(HM_verbose) CALL this%write_parameters(z,lut,cosm)
    !CALL this%write_parameters(z,lut,cosm)
    ...
    !WRITE(command, '(A,F10.5,A,F10.5)') 'echo ', z, ' ', this%delta_c(z, lut, cosm)
    !CALL EXECUTE_COMMAND_LINE(command)  ! Executes "echo <value of z> <value of delta_c>"
    ...
    IF (this%imead==1 .OR. this%imead==3 .OR. this%imead==4 .OR. this%imead==5 .OR. this%imead==6 .OR. this%imead==12 .OR. this%imead==13) THEN
    ...
    SUBROUTINE conc_bull(this,z,collapsed_dimensions,lut,cosm)
    class(THalofit) :: this
    !Calculates the Bullock et al. (2001) concentration-mass relation
    REAL(dl), INTENT(IN) :: z
    INTEGER, INTENT(IN) :: collapsed_dimensions
    ...
    CALL this%zcoll_bull(z,collapsed_dimensions,cosm,lut)
    ...
    IF(z<zinf) THEN
        ...
        IF(this%imead==0 .OR. this%imead==2 .OR. this%imead==3 .OR. this%imead==4 .OR. this%imead==5 .OR. this%imead==6 .OR. this%imead>=10) THEN
        ...
        IF(this%imead==3 .OR. this%imead==4 .OR. this%imead==5 .OR. this%imead==6 .OR. this%imead>=10) THEN
    ...
    SUBROUTINE conc_ludlow(this, z, lut, cosm)
    class(THalofit) :: this
    REAL(dl), INTENT(IN) :: z
    TYPE(HM_cosmology) :: cosm
    TYPE(HM_tables) :: lut
    REAL(dl) :: D_z, c0, beta, gamma1, gamma2, nu, nu0, sigma, a
    INTEGER :: i

    ! Find the growth function at the current redshift
    D_z = grow(z, cosm)
    IF (D_z <= 0.0) THEN
        PRINT *, 'Error: Growth factor D_z is non-positive.'
        RETURN
    END IF
    ...
    ! Calculate nu_0
    nu0 = (4.135 - 0.564 * a**(-1) - 0.210 * a**(-2) &
          + 0.0557 * a**(-3) - 0.00348 * a**(-4)) / D_z

    IF (nu0 == 0.0) THEN
        PRINT *, 'Error: nu0 is zero.'
        RETURN
    END IF

    DO i = 1, lut%n
        ! Check the dimensionless peak height for validity
        nu = lut%nu(i)
        IF (nu == 0.0) THEN
            PRINT *, 'Warning: nu is zero at index', i
        END IF
        
        ! Calculate concentration and fill look-up table
        lut%c(i) = c0 * (nu / nu0)**(-gamma1) * &
                   (1.0 + (nu / nu0)**(1.0 / beta))**(-beta * (gamma2 - gamma1))
    END DO

    END SUBROUTINE conc_ludlow
    ...
    SUBROUTINE zcoll_bull(this,z,collapsed_dimensions,cosm,lut)
    class(THalofit) :: this
    ...
    IF(this%imead==0 .OR. this%imead==4 .OR. this%imead==5 .OR. this%imead==10 .OR. this%imead==11 .OR. frac<1.e-3) THEN
        p_2h=plin
    ELSE IF(this%imead==1 .OR. this%imead==2) THEN
        sigv=lut%sigv
    ...
    ELSE IF(this%imead==6 .OR. this%imead==12 .OR. this%imead==13) THEN
        !!kdamp=0.05699*lut%sig8z_cold**(-1.089)
        !!ndamp=2.85
        !!x=(k/kdamp)**ndamp
        !!p_2h=p_dewiggle(k, lut%z, plin, lut%sigv, cosm)*(1.-frac*x/(1.+x))
        !!p_2h=p_dewiggle(k, lut%z, plin, lut%sigv, cosm)*(1. - k**2*lut%sigv**2 + k**4*lut%sigv**4/2)!-k**6*lut%sigv**6/6)
        !p_2h=p_dewiggle(k, lut%z, plin, lut%sigv, cosm)*exp(- k**2*lut%sigvRnl**2)+k**7*lut%sigQ3Rnl/2
        p_2h=plin
        !!sigv=lut%sigv
        !!p_2h=p_dewiggle(k, lut%z, plin, lut%sigv, cosm)*(1.-0.1*frac*(tanh(k*sigv/sqrt(ABS(frac))))**2.)
    END IF
    ...
    FUNCTION p_1h(this,k,plin,lut,cosm)
    class(THalofit) :: this
    ...
    TYPE(HM_tables), INTENT(IN) :: lut
    TYPE(HM_cosmology), INTENT(IN) :: cosm
    REAL(dl) :: g, gs, gf, fac, et, ks, wk, wkf, wks, wkTH, wk_2, wkf_2, wkfc_2, wks_2, wkTH_2, wk_x, wkf_x, wks_x, x, m, b, bs, bf
    REAL(dl) :: integrand(lut%n)
    REAL(dl) :: integrand_ff(lut%n)
    REAL(dl) :: integrand_ss(lut%n)
    REAL(dl) :: integrand_halo(lut%n)
    REAL(dl) :: integrand_filament(lut%n)
    REAL(dl) :: integrand_sheet(lut%n)
    REAL(dl) :: integrand_spherehalo(lut%n)
    REAL(dl) :: integrand_spherefilament(lut%n)
    REAL(dl) :: integrand_spheresheet(lut%n)
    REAL(dl) :: sum, sum_ff, sum_ss, sum_halo, sum_filament, sum_sheet, sum_spherehalo, sum_spherefilament, sum_spheresheet, sum_halo_cross, sum_filament_cross, sum_sheet_cross
    INTEGER :: i,ithresh
    REAL(dl), PARAMETER :: pi=pi_HM
    ...
    IF(this%imead>=10) THEN
        DO i=1,lut%n
            ...
            gf=gnu(lut%nu(i), 2, this%ivar_fnu)
            gs=gnu(lut%nu(i), 1, this%ivar_fnu)
            integrand_halo(i)=g
            integrand_filament(i)=gf
            integrand_sheet(i)=gs
        END DO
        sum_halo=inttab(lut%nu,integrand_halo,1,lut%n,iorder)
        sum_filament=inttab(lut%nu,integrand_filament,1,lut%n,iorder)
        sum_sheet=inttab(lut%nu,integrand_sheet,1,lut%n,iorder)
        !Set normalisation to 1 if bigger than 1 (this should never happen!)
        IF (sum_halo > 1.0) sum_halo=1.0
        IF (sum_filament > 1.0) sum_filament=1.0
        IF (sum_sheet > 1.0) sum_sheet=1.0
        !sum_halo=1.0
        !sum_filament=1.0
        !sum_sheet=1.0

        
        !IF(HM_verbose .and. k<=0.0001) WRITE(*,*) 'P1H: Sum halo:', sum_halo
        !IF(HM_verbose .and. k<=0.0001) WRITE(*,*) 'P1H: Sum filament:', sum_filament
        !IF(HM_verbose .and. k<=0.0001) WRITE(*,*) 'P1H: Sum sheet:', sum_sheet
        !IF (sum_halo > 1.001) STOP 'P_1H: Error, The mass integral over the halo mass function should be smaller or equal 1.'
        !IF (sum_filament > 1.001) STOP 'P_1H: Error, The mass integral over the filament mass function should be smaller or equal 1.'
        !IF (sum_sheet > 1.001) STOP 'P_1H: Error, The mass integral over the sheet mass function should be smaller or equal 1.'
    END IF
    ...
    DO i=1,lut%n
        ...
        IF(this%imead>=10) THEN
            ...
            gf=gnu(lut%nu(i), 2, this%ivar_fnu)
            gs=gnu(lut%nu(i), 1, this%ivar_fnu)
            !Linear bias calculation not necessary for default WHM calculation
            b=bias(lut%nu(i), 4, lut%dc, (lut%rr(i)/lut%rv(i))**(3._dl), this%ivar_bias)
            !b=bias(lut%nu(i), 3, lut%dc)
            bf=bias(lut%nu(i), 2, lut%dc, (lut%rr(i)/lut%rv(i))**(3._dl), this%ivar_bias)
            bs=bias(lut%nu(i), 1, lut%dc, (lut%rr(i)/lut%rv(i))**(3._dl), this%ivar_bias)
            !b=1.0_dl
            !bf=1.0_dl
            !bs=1.0_dl
            
            !choose this for NFW inspired Window Functions (depending on concentration parameter)
            wk=win(k*lut%nu(i)**et,lut%rv(i),lut%c(i)) !this is the standard NFW window function with all mass within rv
            wk_2=wk*wk
            ...
            IF(this%ivar_profile==3) THEN
                ...
                wks_2=win_alt_2wc(k*lut%nu(i)**0.0,lut%rv(i),lut%c(i),lut%rr(i), 1)
            ...
            ELSE
                ...
                wks_2=win_alt_2w(k*lut%nu(i)**0.0,lut%rv(i),lut%rr(i), 1)
                wkf_x=win_alt_cross(k*lut%nu(i)**0.0,lut%rv(i),lut%rr(i), 2)
                wks_x=win_alt_cross(k*lut%nu(i)**0.0,lut%rv(i),lut%rr(i), 1)
            END IF
            
            wkTH=win_alt(k*lut%nu(i)**0.0,lut%rv(i),lut%rr(i), 0, 0)
            wkTH_2 = wkTH*wkTH
            wk_x = wk*wkTH
            ...
            ELSE IF (this%imead==11 .OR. this%imead==13) THEN
                integrand(i) = g*(wk_2-wkf_2)*lut%m(i)
                integrand_ff(i) = g*(wkf_2-wks_2)*lut%m(i)
                integrand_ss(i) = g*(wks_2-wkTH_2)*lut%m(i)
                integrand_halo(i) = g*wk*b
                integrand_filament(i) = gf*wkf*bf
                integrand_sheet(i) = gs*wks*bs
            ELSE IF (this%ibrieden==1) THEN
                integrand(i)=g*(wk_2)*lut%m(i)
            ELSE IF (this%ibrieden==2) THEN
                integrand(i)=gf*(wkf_2)*lut%m(i)
            ELSE IF (this%ibrieden==3) THEN
                integrand(i)=gs*(wks_2)*lut%m(i)
            ELSE IF (this%ibrieden==4) THEN
                integrand(i)=g*(wkf_2)*lut%m(i)
            ELSE IF (this%ibrieden==5) THEN
                integrand(i)=gf*(wks_2)*lut%m(i)
            ELSE IF (this%ibrieden==6) THEN
                integrand(i)=gs*(wkTH_2)*lut%m(i)
            ELSE IF (this%ibrieden==7) THEN
                integrand(i)=g*(wk_2-wkf_2)*lut%m(i)!*(1.-cosm%f_nu)**2
            ELSE IF (this%ibrieden==8) THEN
                integrand(i)=gf*(wkf_2-wks_2)*lut%m(i)!*(1.-cosm%f_nu)**2
            ELSE IF (this%ibrieden==9) THEN
                integrand(i)=gs*(wks_2-wkTH_2)*lut%m(i)!*(1.-cosm%f_nu)**2
            ELSE IF (this%ibrieden==10) THEN
                integrand(i)=g*wk*b
            ELSE IF (this%ibrieden==11) THEN
                integrand(i)=gf*wkf*bf
            ELSE IF (this%ibrieden==12) THEN
                integrand(i)=gs*wks*bs
            ELSE IF (this%ibrieden==13) THEN
                integrand(i)=g*(wkf)*b
            ELSE IF (this%ibrieden==14) THEN
                integrand(i)=gf*(wks)*bf
            ELSE IF (this%ibrieden==15) THEN
                integrand(i)=gs*(wkTH)*bs
            ELSE
                integrand(i)=(g*(wk_2)+gf*(wkf_2)+gs*(wks_2))*lut%m(i)
            END IF
        ELSE
            !Use this (collapsed_dimensions=4) for Sheth&Tormen 1998
            g=gnu(lut%nu(i), 4, 0)
            !Use this for standard NFW
    ...
    !IF(this%imead==3 .OR. this%imead==4 .OR. this%imead==10) integrand=integrand*(1.-cosm%f_nu)**2
    IF(this%imead==3 .OR. this%imead==4 .OR. this%imead==6 .OR. this%imead==12 .OR. this%imead==13) integrand=integrand*(1.-cosm%f_nu)**2

    !Carries out the integration, adds remaining mass from the lowest mass bin, such that the end result is independent of the input
    !mass range. 
    IF(this%imead>=10) THEN
        IF (this%ibrieden==0) THEN
            IF(this%imead==10 .OR. this%imead==12) THEN
                ithresh = find_threshold_index(lut%nu,4.598_dl)
                !sum=(inttab(lut%nu,integrand,1,lut%n,iorder) &
                sum=(inttab(lut%nu(1:ithresh),integrand(1:ithresh),1,ithresh,iorder) &
                    +((1-sum_halo)*(win(k*lut%nu(1)**et,lut%rv(1),lut%c(1))**2-win_alt_2w(k*lut%nu(1)**et,lut%rv(1),lut%rr(1), 2)) &
                    +(1-sum_filament)*(win_alt_2w(k*lut%nu(1)**0.0,lut%rv(1),lut%rr(1), 2)-win_alt_2w(k*lut%nu(1)**0.0,lut%rv(1),lut%rr(1), 1)) &
                    +(1-sum_sheet)*(win_alt_2w(k*lut%nu(1)**0.0,lut%rv(1),lut%rr(1), 1)-win_alt(k*lut%nu(1)**0.0,lut%rv(1),lut%rr(1), 0, 0)**2)) &
                    *lut%m(1))/cosmic_density(cosm)
            ELSE IF (this%imead==11 .OR. this%imead==13) THEN
                sum=(inttab(lut%nu,integrand,1,lut%n,iorder) &
                    +(1-sum_halo)*(win(k*lut%nu(1)**et,lut%rv(1),lut%c(1))**2-win_alt_2w(k*lut%nu(1)**et,lut%rv(1),lut%rr(1), 2)) &
                    *lut%m(1))/cosmic_density(cosm)
        ...
        ELSE IF (this%ibrieden==1) THEN
            sum=(inttab(lut%nu,integrand,1,lut%n,iorder) &
                +(1-sum_halo)*win(k*lut%nu(1)**et,lut%rv(1),lut%c(1))**2 &
                *lut%m(1))/cosmic_density(cosm)
        ELSE IF (this%ibrieden==2) THEN
            sum=(inttab(lut%nu,integrand,1,lut%n,iorder) &
                +(1-sum_filament)*win_alt_2w(k*lut%nu(1)**0.0,lut%rv(1),lut%rr(1), 2) &
                *lut%m(1))/cosmic_density(cosm)   
        ELSE IF (this%ibrieden==3) THEN
            sum=(inttab(lut%nu,integrand,1,lut%n,iorder) &
                +(1-sum_sheet)*win_alt_2w(k*lut%nu(1)**0.0,lut%rv(1),lut%rr(1), 1) &
                *lut%m(1))/cosmic_density(cosm) 
        ELSE IF (this%ibrieden==4) THEN
            sum=(inttab(lut%nu,integrand,1,lut%n,iorder) &
                +(1-sum_halo)*win_alt_2w(k*lut%nu(1)**0.0,lut%rv(1),lut%rr(1), 2) &
                *lut%m(1))/cosmic_density(cosm)
        ELSE IF (this%ibrieden==5) THEN
            sum=(inttab(lut%nu,integrand,1,lut%n,iorder) &
                +(1-sum_filament)*win_alt_2w(k*lut%nu(1)**0.0,lut%rv(1),lut%rr(1), 1) &
                *lut%m(1))/cosmic_density(cosm)   
        ELSE IF (this%ibrieden==6) THEN
            sum=(inttab(lut%nu,integrand,1,lut%n,iorder) &
                +(1-sum_sheet)*win_alt(k*lut%nu(1)**0.0,lut%rv(1),lut%rr(1), 0, 0)**2 &
                *lut%m(1))/cosmic_density(cosm)
        ELSE IF (this%ibrieden==7) THEN
            sum=(inttab(lut%nu,integrand,1,lut%n,iorder) &
                +(1-sum_halo)*(win(k*lut%nu(1)**et,lut%rv(1),lut%c(1))**2-win_alt_2w(k*lut%nu(1)**0.0,lut%rv(1),lut%rr(1), 2)) &
                *lut%m(1))/cosmic_density(cosm)
        ELSE IF (this%ibrieden==8) THEN
            sum=(inttab(lut%nu,integrand,1,lut%n,iorder) &
                +(1-sum_filament)*(win_alt_2w(k*lut%nu(1)**0.0,lut%rv(1),lut%rr(1), 2)-win_alt_2w(k*lut%nu(1)**0.0,lut%rv(1),lut%rr(1), 1)) &
        ...
        ELSE IF (this%ibrieden==10) THEN
            !sum=inttab(lut%nu,integrand,1,lut%n,iorder)
            sum=inttab(lut%nu,integrand,1,lut%n,iorder) &
                +(1-sum_halo)*win(k*lut%nu(1)**et,lut%rv(1),lut%c(1))*bias(lut%nu(1), 4, lut%dc, (lut%rr(1)/lut%rv(1))**(3._dl), 0) !&
        ELSE IF (this%ibrieden==11) THEN
            !sum=inttab(lut%nu,integrand,1,lut%n,iorder)
            sum=inttab(lut%nu,integrand,1,lut%n,iorder) &
                +(1-sum_filament)*win_alt(k*lut%nu(1)**0.0,lut%rv(1),lut%rr(1), 2, this%ivar_profile)*bias(lut%nu(1), 2, lut%dc, (lut%rr(1)/lut%rv(1))**(3._dl), this%ivar_bias) !&
        ELSE IF (this%ibrieden==12) THEN
            !sum=inttab(lut%nu,integrand,1,lut%n,iorder)
            sum=inttab(lut%nu,integrand,1,lut%n,iorder) &
                +(1-sum_sheet)*win_alt(k*lut%nu(1)**0.0,lut%rv(1),lut%rr(1), 1, this%ivar_profile)*bias(lut%nu(1), 1, lut%dc, (lut%rr(1)/lut%rv(1))**(3._dl), this%ivar_bias) !&
        ELSE IF (this%ibrieden==13) THEN
            !sum=inttab(lut%nu,integrand,1,lut%n,iorder)
            sum=inttab(lut%nu,integrand,1,lut%n,iorder) &
                +(1-sum_halo)*win_alt(k*lut%nu(1)**et,lut%rv(1),lut%rr(1), 2, this%ivar_profile)*bias(lut%nu(1), 4, lut%dc, (lut%rr(1)/lut%rv(1))**(3._dl), this%ivar_bias)
        ELSE IF (this%ibrieden==14) THEN
            !sum=inttab(lut%nu,integrand,1,lut%n,iorder)
            sum=inttab(lut%nu,integrand,1,lut%n,iorder) &
                +(1-sum_filament)*win_alt(k*lut%nu(1)**0.0,lut%rv(1),lut%rr(1), 1, this%ivar_profile)*bias(lut%nu(1), 2, lut%dc, (lut%rr(1)/lut%rv(1))**(3._dl), this%ivar_bias)
        ELSE IF (this%ibrieden==15) THEN
            !sum=inttab(lut%nu,integrand,1,lut%n,iorder)
            sum=inttab(lut%nu,integrand,1,lut%n,iorder) &
                +(1-sum_sheet)*win_alt(k*lut%nu(1)**0.0,lut%rv(1),lut%rr(1), 0, 0)*bias(lut%nu(1), 1, lut%dc, (lut%rr(1)/lut%rv(1))**(3._dl), this%ivar_bias)
        END iF
    ELSE 
    ...
    IF(this%imead==11 .OR. this%imead==13) THEN
        IF (this%ivar_profile==3) THEN
            ...
            sum_filament_cross=inttab(lut%nu,integrand_filament,1,lut%n,iorder) &
                    +(1-sum_filament)*win_alt_nfw(k*lut%nu(1)**0.0,lut%rv(1),lut%c(1),lut%rr(1), 2)*bias(lut%nu(1), 2, lut%dc, (lut%rr(1)/lut%rv(1))**(3._dl), this%ivar_bias)
            sum_sheet_cross=inttab(lut%nu,integrand_sheet,1,lut%n,iorder) &
                    +(1-sum_sheet)*win_alt_nfw(k*lut%nu(1)**0.0,lut%rv(1),lut%c(1),lut%rr(1), 1)*bias(lut%nu(1), 1, lut%dc, (lut%rr(1)/lut%rv(1))**(3._dl), this%ivar_bias)
        ELSE
            sum_halo_cross=inttab(lut%nu,integrand_halo,1,lut%n,iorder) &
                    +(1-sum_halo)*win(k*lut%nu(1)**et,lut%rv(1),lut%c(1))*bias(lut%nu(1), 4, lut%dc, (lut%rr(1)/lut%rv(1))**(3._dl), this%ivar_bias)
            sum_filament_cross=inttab(lut%nu,integrand_filament,1,lut%n,iorder) &
            ...
            sum_sheet_cross=inttab(lut%nu,integrand_sheet,1,lut%n,iorder) &
                    +(1-sum_sheet)*win_alt(k*lut%nu(1)**0.0,lut%rv(1),lut%rr(1), 1, this%ivar_profile)*bias(lut%nu(1), 1, lut%dc, (lut%rr(1)/lut%rv(1))**(3._dl), this%ivar_bias)
        END IF
        sum_ff = (inttab(lut%nu,integrand_ff,1,lut%n,iorder) &
                 +(1-sum_filament)*(win_alt_2w(k*lut%nu(1)**0.0,lut%rv(1),lut%rr(1), 2)-win_alt_2w(k*lut%nu(1)**et,lut%rv(1),lut%rr(1), 1)) &
                 *lut%m(1))/cosmic_density(cosm)
        sum_ss = (inttab(lut%nu,integrand_ss,1,lut%n,iorder) &
                 +(1-sum_sheet)*(win_alt_2w(k*lut%nu(1)**0.0,lut%rv(1),lut%rr(1), 1)-win_alt(k*lut%nu(1)**et,lut%rv(1),lut%rr(1), 0, 0)) &
                 *lut%m(1))/cosmic_density(cosm)
        p_1h=sum*k**3/(2.*pi**2)+((plin*sum_sheet_cross**2+sum_ss*k**3/(2.*pi**2))*sum_filament_cross**2+sum_ff*k**3/(2.*pi**2))*sum_halo_cross**2
    ELSE
    ...
    function find_threshold_index(array, threshold) result(index)
    implicit none
    ...
    end function find_threshold_index
    ...
    FUNCTION win_alt(k,rv,rr,collapsed_dimensions, ivar)
    !Selects the filament/sheet window function (k-space halo profile, collapsed_dimension is 2 or 1 or 0 in case of a sphere). For halos, which also depend on concentration, call win(k,rv,c)!
    REAL(dl) :: win_alt
    REAL(dl), INTENT(IN) :: k, rv, rr
    INTEGER, INTENT(IN) :: collapsed_dimensions, ivar
    REAL(dl), PARAMETER :: pi=pi_HM
    REAL(dl) :: Dv, d

    Dv = (rr/rv)**(3._dl)
     
    IF (collapsed_dimensions==2) THEN
        !For filaments, decide between mass function with radial dependence (default) or constant density (tophat) 
        d = sqrt(2 * pi / 3._dl) * rr / (Dv**(1/3._dl))
        IF (ivar==2) THEN
            win_alt = WindowHaloNFW(k, rr, 1/ (Dv**(1/3._dl)), Dv)
        ELSE IF (ivar==1) THEN
            win_alt = WindowFilamentNew(k, rr)
        ELSE
            win_alt = WindowFilament(k, d, rr)
            !win_alt = WindowTophat(k, rr, Dv**(2/3._dl))
        END IF
    ELSE IF (collapsed_dimensions==1) THEN
        !For sheets, decide between mass function with radial dependence (default) or constant density (tophat) 
        d = 4/3._dl * rr / (Dv**(1/3._dl))
        IF (ivar==2) THEN
            win_alt = WindowHaloNFW(k, rr, 1/ (Dv**(1/3._dl)), Dv)
        ELSE IF (ivar==1) THEN
            win_alt = WindowSheetNew(k, rr)
        ELSE
            win_alt = WindowSheet(k, d, rr)
        END IF
    ELSE
        !For spheres, just call the tophat for an overdensity of 1.
        win_alt = WindowTophat(k, rr, 1._dl)
    END IF
    
    !Correct for the case of disasters (a bit sloppy, not sure if this is ever used) SB: removed the <0 case, as sometimes the window function can go negative and that's fine.
    IF(win_alt>1._dl) win_alt=1._dl
    !IF(win_alt<0._dl) win_alt=0._dl

    END FUNCTION win_alt

    
    FUNCTION win_alt_2w(k,rv,rr,collapsed_dimensions)
    !Selects the filament/sheet window function (squared!) (k-space profile, collapsed_dimension is 2 or 1 or 0 in case of a sphere). For halos, which also depend on concentration, call win(k,rv,c)!
    REAL(dl) :: win_alt_2w
    REAL(dl), INTENT(IN) :: k, rv, rr
    INTEGER, INTENT(IN) :: collapsed_dimensions
    REAL(dl), PARAMETER :: pi=pi_HM
    ...
    IF (collapsed_dimensions==2) THEN
        !For filaments, decide between mass function with radial dependence (default) or constant density (tophat) 
        win_alt_2w = WindowFilament_2f(k, d, rr)
    ELSE IF (collapsed_dimensions==1) THEN
        !For sheets, decide between mass function with radial dependence (default) or constant density (tophat) 
        win_alt_2w = WindowSheet_2s(k, d, rr)
    ELSE
        !For spheres, just call the tophat for an overdensity of 1.
        win_alt_2w = WindowTophat(k, rr, 1._dl)**2
    END IF
    !Correct for the case of disasters (a bit sloppy, not sure if this is ever used)
    IF(win_alt_2w>1._dl) win_alt_2w=1._dl
    IF(win_alt_2w<0._dl) win_alt_2w=0._dl

    END FUNCTION win_alt_2w

    FUNCTION win_alt_2wc(k,rv,c,rr,collapsed_dimensions)
    !Selects the filament/sheet window function (squared!) (k-space profile, collapsed_dimension is 2 or 1 or 0 in case of a sphere). For halos, which also depend on concentration, call win(k,rv,c)!
    REAL(dl) :: win_alt_2wc
    REAL(dl), INTENT(IN) :: k, rv, c, rr
    INTEGER, INTENT(IN) :: collapsed_dimensions
    REAL(dl), PARAMETER :: pi=pi_HM
    REAL(dl) :: Dv, d
    ...
    IF (collapsed_dimensions==2) THEN
        !For filaments, decide between mass function with radial dependence (default) or constant density (tophat) 
        win_alt_2wc = WindowFilament_2fc(k, d, rr, c)
    ELSE IF (collapsed_dimensions==1) THEN
        !For sheets, decide between mass function with radial dependence (default) or constant density (tophat) 
        win_alt_2wc = WindowSheet_2sc(k, d, rr, c)
    ELSE
        !For spheres, just call the tophat for an overdensity of 1.
        win_alt_2wc = WindowTophat(k, rr, 1._dl)**2
    END IF
    !Correct for the case of disasters (a bit sloppy, not sure if this is ever used)
    IF(win_alt_2wc>1._dl) win_alt_2wc=1._dl
    IF(win_alt_2wc<0._dl) win_alt_2wc=0._dl

    END FUNCTION win_alt_2wc


    FUNCTION win_alt_cross(k,rv,rr,collapsed_dimensions)
    !Selects the filament/sheet window function (squared!) (k-space profile, collapsed_dimension is 2 or 1 or 0 in case of a sphere). For halos, which also depend on concentration, call win(k,rv,c)!
    REAL(dl) :: win_alt_cross
    REAL(dl), INTENT(IN) :: k, rv, rr
    INTEGER, INTENT(IN) :: collapsed_dimensions
    REAL(dl), PARAMETER :: pi=pi_HM
    REAL(dl) :: Dv, d
    ...
    IF (collapsed_dimensions==2) THEN
        !For filaments, decide between mass function with radial dependence (default) or constant density (tophat) 
        win_alt_cross = WindowFilament_cross_Sphere(k, d, rr)
    ELSE IF (collapsed_dimensions==1) THEN
        !For sheets, decide between mass function with radial dependence (default) or constant density (tophat) 
        win_alt_cross = WindowSheet_cross_Sphere(k, d, rr)
    ELSE
        !For spheres, just call the tophat for an overdensity of 1.
        win_alt_cross = WindowTophat(k, rr, 1._dl)**2
    END IF
    !Correct for the case of disasters (a bit sloppy, not sure if this is ever used)
    !IF(win_alt_cross>1._dl) win_alt_cross=1._dl
    !IF(win_alt_cross<0._dl) win_alt_cross=0._dl

    END FUNCTION win_alt_cross
    
    FUNCTION win_alt_nfw(k,rv,c,rr,collapsed_dimensions)
    !Selects the halo/filament/sheet window function (k-space density profile, collapsed_dimension is 3 or 2 or 1 or 0 in case of a sphere). The halos here extend up to radius R! For standard NFW halos, which extend up to rv, call win(k,rv,c)!
    REAL(dl) :: win_alt_nfw
    REAL(dl), INTENT(IN) :: k, rv, c, rr
    INTEGER, INTENT(IN) :: collapsed_dimensions
    REAL(dl), PARAMETER :: pi=pi_HM
    ...
    Dv = (rr/rv)**(3._dl)
    IF (collapsed_dimensions==3) THEN
        !For haloes 
        win_alt_nfw = WindowHaloNFW(k, rr, c, Dv)
    ELSE IF (collapsed_dimensions==2) THEN
        !For filaments 
        win_alt_nfw = WindowFilamentNFW(k, rr, c, Dv)
    ELSE IF (collapsed_dimensions==1) THEN
        !For sheets 
        win_alt_nfw = WindowSheetNFW(k, rr, c, Dv)
    ELSE
        !For spheres, just call the tophat for an overdensity of 1.
        win_alt_nfw = WindowTophat(k, rr, 1._dl)
    END IF
    
    !Correct for the case of disasters (a bit sloppy, not sure if this is ever used) SB: removed the <0 case, as sometimes the window function can go negative and that's fine.
    IF(win_alt_nfw>1._dl) win_alt_nfw=1._dl
    !IF(win_alt_nfw<0._dl) win_alt_nfw=0._dl

    END FUNCTION win_alt_nfw
    ...
    FUNCTION WindowSheet_2s(k, d, R)
      REAL(dl), INTENT(in) :: k, d, R
      REAL :: Wcyl, Hgr !SB Added Wcyl, Hgr
      COMPLEX(dl) :: z_arg, chg !SB added this line
      REAL(dl) :: WindowSheet_2s

      if (k*R<0.01_dl) then
        Wcyl = 1.0_dl
      else
        z_arg = cmplx(0.0_dl, k*R)
        CALL cchg(1.5_dl, 3.0_dl, 4.0_dl*z_arg, chg)
        Hgr = REAL(exp(-2*z_arg)*chg)
        Wcyl = (2.0_dl - 2.0_dl*Hgr)/(k*R)**2
      end if
      
      WindowSheet_2s = Wcyl * ABS(WindowTophat(k, 2.*sqrt(5.)/(3.*sqrt(3.))*d, 1._dl))**2
      !WindowSheet_2s = Wcyl * ABS(WindowTophat(k, 5./6.*d, 1._dl))**2

    END FUNCTION WindowSheet_2s

    FUNCTION WindowSheet_2sc(k, d, R, c)
      REAL(dl), INTENT(in) :: k, d, R, c
      REAL :: Wcyl, Hgr !SB Added Wcyl, Hgr
      COMPLEX(dl) :: z_arg, chg !SB added this line
      REAL(dl) :: WindowSheet_2sc
      ...
      WindowSheet_2sc = Wcyl * winnfw(k,d,c)**2

    END FUNCTION WindowSheet_2sc

    FUNCTION WindowSheet_cross_Sphere(k, d, R)
      REAL(dl), INTENT(in) :: k, d, R
      REAL(dl) :: WindowSheet_cross_Sphere
      REAL(dl), PARAMETER :: pi=pi_HM

      WindowSheet_cross_Sphere = (12*R*k*sin(R*k/2.0_dl)**2*(sin(R*k)-R*k*cos(R*k))/(R*k)**6) * WindowTophat(k, d, 1._dl)**2

    END FUNCTION WindowSheet_cross_Sphere

    FUNCTION WindowFilament_2f(k, d, R)
      REAL(dl), INTENT(in) :: k, d, R
      REAL(dl) :: WindowFilament_2f
      REAL(dl), PARAMETER :: pi=pi_HM

      WindowFilament_2f = 2 * (-1 + cos(2*k*R) + 2*k*R*Si(2*k*R)) / (2*k*R)**2 * ABS(WindowTophat(k, sqrt(5./6.)*d, 1._dl))**2

    END FUNCTION WindowFilament_2f

    FUNCTION WindowFilament_2fc(k, d, R, c)
      REAL(dl), INTENT(in) :: k, d, R, c
      REAL(dl) :: WindowFilament_2fc
      REAL(dl), PARAMETER :: pi=pi_HM

      WindowFilament_2fc = 2 * (-1 + cos(2*k*R) + 2*k*R*Si(2*k*R)) / (2*k*R)**2 * winnfw(k,d,c)**2

    END FUNCTION WindowFilament_2fc

    FUNCTION WindowFilament_cross_Sphere(k, d, R)
      REAL(dl), INTENT(in) :: k, d, R
      REAL(dl) :: WindowFilament_cross_Sphere
      REAL(dl), PARAMETER :: pi=pi_HM

      WindowFilament_cross_Sphere = (3*(sin(k*R) - k*R*cos(k*R))*Si(k*R)/R**4/k**4) * WindowTophat(k, d, 1._dl)**2

    END FUNCTION WindowFilament_cross_Sphere
    
    FUNCTION WindowFilament(k, d, R)
      REAL(dl), INTENT(in) :: k, d, R
      REAL(dl) :: WindowFilament
      REAL(dl), PARAMETER :: pi=pi_HM

      WindowFilament = -((3*sqrt(pi) * (-d * k * sqrt(2 * pi) * cos((d * k)/sqrt(2 * pi)) + &
                       2 * pi * sin((d * k)/sqrt(2 * pi)) - &
                       d**2 * k**2 * Si((d * k)/sqrt(2 * pi)) + &
                       d**2 * k**2 * Si(k * R))) / &
                       (d**2 * k**3 * (sqrt(2.0_dl) * d - 3 * sqrt(pi) * R)))

    END FUNCTION WindowFilament

    FUNCTION WindowSheet(k, d, R)
      REAL(dl), INTENT(in) :: k, d, R
      REAL(dl) :: WindowSheet

      WindowSheet = (24 * (d * k * cos(k * R) - 2 * sin((d * k)/2))) / (k**3 * (d**3 - 12 * d * R**2))

    END FUNCTION WindowSheet

    FUNCTION WindowFilamentNew(k, R)
      REAL(dl), INTENT(in) :: k, R
      REAL(dl) :: WindowFilamentNew
      REAL(dl), PARAMETER :: pi=pi_HM

      WindowFilamentNew = Si(k * R) / (k * R)

    END FUNCTION WindowFilamentNew

    FUNCTION WindowSheetNew(k, R)
      REAL(dl), INTENT(in) :: k, R
      REAL(dl) :: WindowSheetNew

      WindowSheetNew = (2 - 2*cos(k * R))/(k * R)**2

    END FUNCTION WindowSheetNew
    
    function WindowHaloNFW(k, R, c, Deltav) result(output)
    real(dl) :: c, k, R, Deltav, output
    real(dl), parameter :: pi=pi_HM

    ! Intermediate variables
    real(dl) :: rv, temp1, temp2, temp3, temp4, temp5
    ...
    output = -(((temp1 + rv) * (k * cos(temp2) * Ci(temp3) - temp4 / temp5 - &
              k * (cos(temp2) * Ci(temp2) + sin(temp2) * Si(temp2)) + &
              k * sin(temp2) * Si(temp3))) / &
              (k * (temp1 + (temp1 + rv) * log(rv / (temp1 + rv)))))

    end function WindowHaloNFW
    
    function WindowFilamentNFW(k, R, c, Deltav) result(output)
    real(dl) :: k, R, c, Deltav, output

    ! Intermediate variables
    real(dl) :: temp1, temp2, temp3, temp4, temp5, temp6, temp7, temp8

    temp1 = c * Deltav**(1.0/3.0)
    temp2 = k * R / (c * Deltav**(1.0/3.0))
    temp3 = k * (R + R / (c * Deltav**(1.0/3.0)))
    temp4 = k * R
    temp5 = 1.0 + c * Deltav**(1.0/3.0)
    temp6 = 1.0 / temp5
    temp7 = 2.0 * log(R)
    temp8 = 2.0 * log(R + c * Deltav**(1.0/3.0) * R)

    output = (c * Deltav**(1.0/3.0) * sin(k * R) * temp6 + &
               Ci(temp2) * (temp4 * cos(temp2) + c * Deltav**(1.0/3.0) * sin(temp2)) - &
               Ci(temp3) * (temp4 * cos(temp2) + c * Deltav**(1.0/3.0) * sin(temp2)) - &
               (c * Deltav**(1.0/3.0) * cos(temp2) - temp4 * sin(temp2)) * Si(temp2) + &
               (c * Deltav**(1.0/3.0) * cos(temp2) - temp4 * sin(temp2)) * Si(temp3)) / &
             (temp4 * (1.0 - temp6 + c * Deltav**(1.0/3.0) + temp7 - temp8))

    end function WindowFilamentNFW
    
    function WindowSheetNFW(k, R, c, Deltav) result(output)
    real(dl) :: k, R, c, Deltav, output

    ! Intermediate variables
    real(dl) :: temp1, temp2, temp3, temp4, temp5

    temp1 = c * Deltav**(1.0/3.0)
    temp2 = k * R / (c * Deltav**(1.0/3.0))
    temp3 = (c + 1.0/Deltav**(1.0/3.0)) * k * R / c
    temp4 = k**2 * R**2
    temp5 = c * (-2.0 + c * Deltav**(1.0/3.0)) * Deltav**(1.0/3.0) + 2.0 * log(1.0 + c * Deltav**(1.0/3.0))

    output = (2.0 * c * Deltav**(1.0/3.0) * (-c * Deltav**(1.0/3.0) * (-1.0 + cos(k * R)) + &
              k * R * (Ci(temp3) - Ci(temp2)) * sin(temp2) + &
              k * R * cos(temp2) * (Si(temp2) - Si(temp3)))) / (temp4 * temp5)

    end function WindowSheetNFW

    function WindowFilamentNFWNew(k, R, c, Deltav) result(output)
    real(dl) :: k, R, c, Deltav, output
    ...
    temp1 = c * Deltav**(1.0/3.0)
    temp2 = (c + 1.0/Deltav**(1.0/3.0)) * k * R / c
    temp3 = k * R / (c * Deltav**(1.0/3.0))
    temp4 = c * Deltav**(1.0/3.0) * k * R
    temp5 = k * R * log(R) - k * R * log(R + c * Deltav**(1.0/3.0) * R)

    output = (c * Deltav**(1.0/3.0) * ((-Ci(temp2) + Ci(temp3)) * Sin(temp3) + &
              Cos(temp3) * (Si(temp2) - Si(temp3)))) / (temp4 + temp5)

    end function WindowFilamentNFWNew

    FUNCTION WindowTophat(k, R, Deltav)
      REAL(dl), INTENT(in) :: k, R, Deltav
      REAL(dl) :: WindowTophat

      WindowTophat = (3 * Deltav) / (k * R)**3 * &
                     (sin(k * R / (Deltav**(1.0/3.0))) - &
                     (k * R / (Deltav**(1.0/3.0))) * cos(k * R / (Deltav**(1.0/3.0))))

    END FUNCTION WindowTophat

    FUNCTION bias(nu, collapsed_dimensions, deltac, Deltav, ivar)
    !Select the mass function
    REAL(dl) :: bias
    REAL(dl), INTENT(IN) :: nu, deltac, Deltav
    INTEGER, INTENT(IN) :: collapsed_dimensions, ivar
    REAL(dl) :: A1, B1, C1, a2, b2, c2, y2
 
    ! These first three functions for halos, filaments and sheets come from Shen, Abel, Mo, Sheth (2006)
    IF (ivar==1) THEN
        bias = 1._dl
    ELSE
        IF (collapsed_dimensions==3) THEN
            ...
            ELSE IF (ivar==3) THEN
                y2 = LOG10(Deltav)
                A1 = 1.0 + 0.24*y2*exp(-(4/y2)**4)
                a2 = 0.44*y2-0.88
                B1 = 0.183
                b2 = 1.5
                C1 = 0.019+0.107*y2+0.19*exp(-(4/y2)**4)
                c2 = 2.4
                bias = 1.0 - A1 * nu**a2/(nu**a2+deltac**a2) +B1*nu**b2 + C1*nu**c2
            ELSE
                bias = 1.0 - 1.0 / deltac * (1.0 - ((0.1089900326441962 - &
                0.004295387899174991 / (nu**2)**0.21999999999999997 - &
                0.02069023105554754 * (nu**2)**0.3899999999999999 + &
                0.3697666444572139 * (nu**2)**1.0000000000000002 + &
                0.707 * (nu**2)**1.61) / (0.08933609233130838 + &
                (nu**2)**0.61)))   
            END IF
        ELSE IF (collapsed_dimensions==2) THEN
            ! For filaments:
            bias = 1.0 - 1.0 / deltac * (1.0 + (0.003649768991022675 + &
            3.5451699602011876E-7 * (nu**2)**0.43999999999999995 - &
            0.00014213618961920587 * (nu**2)**0.72 + &
            0.018070333344055185 * (nu**2)**1.0 - &
            0.707 * (nu**2)**1.28) / (-0.0065174446268262045 + &
            (nu**2)**0.28))
        ELSE IF (collapsed_dimensions==1) THEN
            ! For sheets:
            bias = 1.0 - 1.0 / deltac * (1.0 + (1.0 * (0.15245599087020684 - &
            0.004499724478696993 / (nu**2)**0.10000000000000009 - &
            0.027295045644610594 * (nu**2)**0.44999999999999996 + &
            0.5291782113456397 * (nu**2)**1.0000000000000002 - &
            0.7070000000000001 * (nu**2)**1.55)) / (-0.13859635533655162 + &
            (nu**2)**0.55))
            IF (nu<0.4371_dl) bias=0._dl
        ELSE
            IF (ivar==2) THEN
                bias = 1._dl
            ELSE IF (ivar==3) THEN
                ...
                bias = 1.0 - A1 * nu**a2/(nu**a2+deltac**a2) +B1*nu**b2 + C1*nu**c2
            ELSE
                !Standard Sheth & Torman (1999)
                bias = 1.0 - 1.0 / deltac * (1.0 + (-0.6657718465770451 - &
                0.7845011592166181 * (nu**2)**1.0 - &
                0.7069999999999999 * (nu**2)**1.3) / (1.1096197442950755 + &
                (nu**2)**0.30000000000000004)) 
            END IF
        END IF
    END IF

    !IF(bias<0._dl) bias=0._dl
    
    END FUNCTION bias

    FUNCTION gnu(nu, collapsed_dimensions, ivar)
    !Select the mass function
    REAL(dl) :: gnu
    ...
    IF (collapsed_dimensions==3) THEN
        ! For halos:
        gnu=gst_general(nu, 0.707_dl, 0.61_dl, 0.45_dl, 0.639_dl) 
    ELSE IF (collapsed_dimensions==2) THEN
        ! For filaments:
        gnu=gst_general(nu, 0.707_dl, 0.28_dl, -0.012_dl, 0.639_dl/0.951401_dl)
    ELSE IF (collapsed_dimensions==1) THEN
        ! For sheets:
        gnu=gst_general(nu, 0.707_dl, 0.55_dl, -0.56_dl, 0.639_dl/1.0122_dl)
    ELSE
        !Standard Sheth & Torman (1999)
        IF (ivar==0) THEN
            gnu=gst(nu)
            !gnu=gdesp(nu)
        ELSE IF (ivar==1) THEN
            gnu=gst(nu)/0.983561_dl/0.9780787_dl
            !gnu=gst(nu)
        ELSE
            gnu=gst(nu)/0.983561_dl
    ...
    REAL(dl), PARAMETER :: a=0.7689
    REAL(dl), PARAMETER :: bigA=0.262903
    !REAL(dl), PARAMETER :: p=0.2089
    !REAL(dl), PARAMETER :: a=0.7369
    !REAL(dl), PARAMETER :: bigA=0.310696
    ...
    !Full mass function. Note this is normalised such that integral f(nu)dnu = 1
    gdesp=bigA*(1+((a*nu*nu)**(-p)))*exp(-a*nu*nu/2)
    !IF(nu<0.0025_dl) gdesp=0._dl
    IF(nu<0.066_dl) gdesp=0._dl

    END FUNCTION gdesp
    
    FUNCTION gwatson(sigma)
    REAL(dl) :: gwatson
    REAL(dl), INTENT(IN) :: sigma
    REAL(dl), PARAMETER :: A=0.282
    REAL(dl), PARAMETER :: alpha=2.163
    REAL(dl), PARAMETER :: beta=1.406
    REAL(dl), PARAMETER :: gamma=1.210

    gwatson = A * ((beta / sigma) ** alpha + 1.0_8) * exp(-gamma / sigma**2)
    
    END FUNCTION gwatson

    FUNCTION gwarren(sigma)
    REAL(dl) :: gwarren
    REAL(dl), INTENT(IN) :: sigma
    REAL(dl), PARAMETER :: A=0.7234
    REAL(dl), PARAMETER :: alpha=1.625
    REAL(dl), PARAMETER :: beta=0.2538
    REAL(dl), PARAMETER :: gamma=1.1982

    gwarren = A * ((1 / sigma) ** alpha + beta) * exp(-gamma / sigma**2)
    
    END FUNCTION gwarren

    
    FUNCTION gst_general(nu, a, alpha, beta, bigA)
    !Shen, Abel, Mo, Sheth (2006) mass function!
    REAL(dl) :: gst_general
    REAL(dl), INTENT(IN) :: nu, a, alpha, beta, bigA

    !Note I use nu=dc/sigma whereas ST (1999) use nu=(dc/sigma)^2
    !This accounts for the different pre-factor and slighly changed nu dependence
    !f(nu^2)d(nu^2)=2*nu*f(nu)dnu

    !Full mass function. Note this expression can go negative, which is unphysical. Hence the if statement at the end.
    gst_general = bigA * exp(-a*nu**2*(1 + beta/(a*nu**2)**alpha)**2/2) * &
             (1 + beta/(a*nu**2)**alpha * (1 - alpha + (alpha*(alpha - 1)/2.0) - &
             (alpha*(alpha - 1)*(alpha - 2)/6.0) + (alpha*(alpha - 1)*(alpha - 2)* &
             (alpha - 3)/24.0) - (alpha*(alpha - 1)*(alpha - 2)*(alpha - 3)* &
             (alpha - 4)/120.0)))
    
    IF(gst_general<0._dl) gst_general=0._dl

    END FUNCTION gst_general
    ...
    FUNCTION sigmaQ3(R,z,kmin,kmax,itype,cosm)
    REAL(dl) :: sigmaQ3
    REAL(dl), INTENT(IN) :: R
    REAL(dl), INTENT(IN) :: z
    REAL(dl), INTENT(IN) :: kmin
    REAL(dl), INTENT(IN) :: kmax
    INTEGER, INTENT(IN) :: itype
    TYPE(HM_cosmology), INTENT(IN) :: cosm
    !REAL(dl), PARAMETER :: tmin=1/(kmax**(1/alpha_sigmaV_integration)+1)
    !REAL(dl), PARAMETER :: tmax=1/(kmin**(1/alpha_sigmaV_integration)+1)
    REAL(dl) :: tmin, tmax
    REAL(dl), PARAMETER :: acc=acc_sigmaV_integration
    REAL(dl), PARAMETER :: alpha=alpha_sigmaV_integration !Speeds up integral for large 'R'
    INTEGER, PARAMETER :: iorder=iorder_sigmaV_integration
    
    tmin=1/(kmax**(1/alpha)+1)
    tmax=1/(kmin**(1/alpha)+1)
    sigmaQ3=integrate(tmin,tmax,sigmaQ3_integrand,R,z,itype,cosm,acc,iorder)/5

    END FUNCTION sigmaQ3
    ...
    FUNCTION sigmaQ3_integrand(t,R,z,itype,cosm)
    !This is the integrand for the velocity dispersion integral
    REAL(dl) :: sigmaQ3_integrand
    REAL(dl), INTENT(IN) :: t
    REAL(dl), INTENT(IN) :: R
    REAL(dl), INTENT(IN) :: z
    INTEGER, INTENT(IN) :: itype
    TYPE(HM_cosmology), INTENT(IN) :: cosm
    REAL(dl) :: k, kR, w_hat, tmax
    REAL(dl), PARAMETER :: alpha=alpha_sigmaV_integration !Speeds up integral for large 'R'
    ...
    ELSE
        ...
        END IF
        w_hat=wk_tophat(kR)
        sigmaQ3_integrand=(p_lin(k,z,itype,cosm)**2/k**7)*(w_hat**2)*alpha/(t*(1-t))
    END IF

    END FUNCTION sigmaQ3_integrand
    ...
    IF(HM_verbose) WRITE(*,*) 'GROWTH: Minimum G(a=0.001):', cosm%growth(1)
    IF(HM_verbose) WRITE(*,*) 'GROWTH: Maximum G(a=0.001):', cosm%growth(n)
